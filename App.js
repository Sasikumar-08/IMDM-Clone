...
function Search() {
  ...
  useEffect(() => {
    const trendingMovies = ["Dune", "Oppenheimer", "John Wick", "Barbie", "Spider-Man"];
    const randomMovie = trendingMovies[Math.floor(Math.random() * trendingMovies.length)];

    fetch(`${API_URL}&s=${randomMovie}`)
      .then((res) => res.json())
  ...
  const handleSearch = async (e) => {
    ...
    if (e.target.value.length > 2) {
      const res = await fetch(`${API_URL}&s=${e.target.value}`);
  ...
  return (
    <div className="container text-center mt-4">
      <h2 className="text-warning fw-bold">🎬 Find Your Favorite Movies</h2>
      <input
        type="text"
      ...
      {/* Trending Movies */}
      <h3 className="text-success mt-5">🔥 Popular Movies Right Now</h3>
      <div className="row mt-3 justify-content-center">
        {trending.map((movie) => (
          <div className="col-lg-3 col-md-4 col-sm-6 mb-4" key={movie.imdbID}>
            <div className="card shadow-lg rounded border-0">
